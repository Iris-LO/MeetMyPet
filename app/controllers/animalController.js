...
const animalController = {
    ...
    findAllGroups : async (request, response) => {
        try {
            ...
            let groupsWithTotalMembers = animalHasGroups; 
            for (let index = 0; index < groupsWithTotalMembers.length; index++) {
                groupsWithTotalMembers[index].totalMembers = await Group.findTotalMembers(groupsWithTotalMembers[index].id);
                
            }
            // groupsWithTotalMembers.forEach (async element => {
            //     element['totalMembers'] = await Group.findTotalMembers(element.id);
            //     console.log(element);
            // });
            console.log(`Groupe avec un nombre : `, groupsWithTotalMembers);
            if (groupsWithTotalMembers[0].totalMembers ) {
                response.status(200).json((`Les groupes dont vous êtes membre.`, groupsWithTotalMembers));
            }
        } catch (error) {
            console.log(error);
    ...
    updateProfile : async (req, res) => {
        const source = req.params.id ? req.params.id : req.body.animal_id;
        const animal = await Animal.findOne(source);
        if (animal.id) { 
            let updatedAnimal = new Animal(req.body);
            updatedAnimal.id = animal.id;
            if (req.params.id) {
                updatedAnimal.image = animal.image;
                await updatedAnimal.save();
            } else if (req.body.animal_id) {
                updatedAnimal = new Animal(animal);
                updatedAnimal.id = req.body.animal_id;
                console.log(req.res.locals.file);
                updatedAnimal.image = req.res.locals.fileName;
                await updatedAnimal.save();
            };
            req.session.user.info.image = updatedAnimal.image;
            if (updatedAnimal) {
